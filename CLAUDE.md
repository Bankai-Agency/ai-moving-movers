...
- **Эмодзи в интерфейсе** — НИКАКИХ эмодзи (⭐, 🔥, ✅, 📦 и т.д.) в UI. Для визуальных маркеров использовать строго SVG-иконки. Юникод-символы (★, •) тоже заменять на SVG.
- **Градиенты на фонах виджетов/карточек** — `linear-gradient` запрещён для фонов основных виджетов, карточек, секций. Градиент допустим только: (1) анимированные фоны/загрузки, (2) декоративные элементы типа прогресс-баров, (3) крайне редко — промо-баннеры. Все карточки — `#FFFFFF`, без градиентов.
